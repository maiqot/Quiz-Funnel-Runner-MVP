# Quiz Funnel Runner

CLI-инструмент для автоматического прохождения quiz-воронок в мобильной эмуляции (**iPhone 13**) со скриншотами и детекцией paywall.

---

## 📑 Оглавление

* [🧱 Tech Stack](#-tech-stack)
* [🚀 Быстрый старт](#-быстрый-старт)
* [▶️ Запуск](#-запуск)
* [🌐 Проверка своих ссылок](#-проверка-своих-ссылок)
* [📂 Результаты](#-результаты)
* [🔍 Quick Verification](#-quick-verification)
* [🧠 Что делает инструмент](#-что-делает-инструмент)
* [⚙️ Настройки](#️-настройки)
* [🏗 Архитектура](#-архитектура)
* [🎯 Подход и обоснование](#-подход-и-обоснование)
* [🧪 Качество решения](#-качество-решения)
* [🚀 Возможные улучшения](#-возможные-улучшения)

---

## 🧱 Tech Stack

* **Node.js**
* **TypeScript**
* **Playwright**
* Mobile emulation: **iPhone 13**
* CLI execution (Terminal / VS Code / Cursor / WebStorm)

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🚀 Быстрый старт

### 1️⃣ Клонирование

```bash
git clone https://github.com/maiqot/Quiz-Funnel-Runner-MVP.git
cd Quiz-Funnel-Runner-MVP
```

### 2️⃣ Установка

```bash
npm install
npx playwright install
```

Скачиваются:

* **Chromium** (по умолчанию)
* **WebKit (Safari)** — для `--safari`

### 3️⃣ Запуск

```bash
npm run dev
```

Результаты появятся в папке:

```
results/
```

[⬆ Вернуться к оглавлению](#-оглавление)

---

## ▶️ Запуск

### Chromium (headless)

```bash
npm run dev
```

### С окном браузера

```bash 
npm run dev -- https://coursiv.io/dynamic --headful
```

### Safari (WebKit)

```bash
npm run dev -- https://coursiv.io/dynamic --safari --headful
```

Во всех режимах используется мобильная эмуляция **iPhone 13**.

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🌐 Проверка своих ссылок

### Через CLI (рекомендуется)

Один URL:

```bash
npm run dev -- https://coursiv.io/dynamic
```

Несколько URL:

```bash
npm run dev -- https://coursiv.io/dynamic?prc_id=1069 https://dance-bit.com/welcomeBellyRef
```

CLI-URL имеют приоритет над `src/config.ts`.

### Через конфигурацию

Отредактируйте:

```
src/config.ts
```

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 📂 Результаты

### По воронкам

```
results/<funnel-slug>/
  01_question.png
  02_input.png
  ...
  summary.json
  log.txt
```

### По типам экранов

```
results/_classified/question/
results/_classified/input/
results/_classified/email/
results/_classified/paywall/
results/_classified/info/
results/_classified/other/
```

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🔍 Quick Verification

### Чистый запуск

```bash
rm -rf results 
// команда удалит результаты 
npm run dev -- https://coursiv.io/dynamic --headful
```

### Проверка paywall

```bash
grep -Ri "paywall detected" results/*/log.txt
```

### Проверка структуры

```bash
ls results
cat results/summary.json
ls results/_classified/paywall
```

Ожидаемо:

* есть `_classified`
* есть `summary.json`
* есть paywall PNG
* `funnelsReachedPaywall: 1`

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🧠 Что делает инструмент

Для каждого URL:

1. Открывает страницу в мобильной эмуляции
2. Классифицирует экран
3. Делает скриншот
4. Логирует действия
5. Выполняет переход
6. Останавливается на paywall

Типы экранов:

* `question`
* `input`
* `email`
* `info`
* `paywall`
* `other`

[⬆ Вернуться к оглавлению](#-оглавление)

---

## ⚙️ Настройки

В `src/config.ts` можно изменить:

* список URL
* максимальное число шагов
* защиту от зацикливания
* дефолтные значения input/email

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🏗 Архитектура

```
src/
  runner/       – управление сценарием
  classifier/   – определение типа экрана
  navigator/    – действия по типу
  utils/        – логирование и файловая система
```

Архитектура построена по принципу разделения ответственности.

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🎯 Подход и обоснование

### Подход

Инструмент реализован как управляемый цикл:

1. Открытие страницы (mobile emulation)
2. Классификация
3. Скриншот
4. Действие
5. Проверка перехода
6. Остановка на paywall

Это делает поведение:

* предсказуемым
* воспроизводимым
* легко расширяемым

---

### Почему TypeScript

* строгий `ScreenType`
* state-machine логика
* безопасность рефакторинга
* снижение runtime-ошибок

---

### Почему Playwright

* мобильная эмуляция
* стабильная работа со SPA
* быстрый запуск
* поддержка Chromium + WebKit

---

### Почему CLI

* интеграция в CI
* простота использования
* отсутствие UI-зависимостей

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🧪 Качество решения

### Надёжность

* защита от зацикливания
* retry действий
* обработка popups / cookies
* fallback submit для email

### Воспроизводимость

* детерминированные эвристики
* CLI-аргументы
* агрегированный `summary.json`

### Расширяемость

Можно добавить:

* LLM fallback
* мульти-стратегию ответов
* экспорт отчётов
* A/B сравнение веток

[⬆ Вернуться к оглавлению](#-оглавление)

---

## 🚀 Возможные улучшения

* LLM-классификация
* сравнение веток funnel
* HTML / PDF отчёт
* CI-интеграция
* аналитика глубины прохождения

[⬆ Вернуться к оглавлению](#-оглавление)

---
